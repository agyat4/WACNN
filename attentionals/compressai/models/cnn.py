...
class WACNN(CompressionModel):
    ...
    def __init__(self, N=192, M=320, **kwargs):
        ...
        '''self.conv3d = nn.Sequential(
            nn.Conv3d(3, 9, kernel_size=(1, 1, 3), stride=1, padding=(0, 0, 1)),
            nn.ReLU(),
            nn.Conv3d(9, 3, kernel_size=(1, 1, 3), stride=1, padding=(0, 0, 1)),
            nn.ReLU(),
        )

        self.spatial_first = nn.Conv2d(9,9, kernel_size=3, stride=1, padding=1)'''




        self.g_a = nn.Sequential(
            conv(9, N, kernel_size=5, stride=2),
            GDN(N),
    ...
    def forward(self, x):
        ...
        expanded= x.reshape(batch_dim,3,3,*spatial_dims)
        #print("expanded shape",expanded.shape)
        x1 = self.conv3d(expanded)
        #print("x1 ko shape ", x1.shape)


        #print("batch extracted shape",batch_dim)

        #Reshape the tensor to (16, 162, 512, 512)
        x1_reshaped = x1.reshape(batch_dim, 9, *spatial_dims)




        x2 = self.spatial_first(x)
        #print("x2 ko shape ", x2.shape)

        x3 = torch.cat([x2, x1_reshaped], dim=1)
